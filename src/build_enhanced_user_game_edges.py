...
DEFAULT_INPUT = Path("data/processed/user_game_edges.csv")
DEFAULT_EDGES_OUTPUT = Path("data/processed/user_game_edges_enhanced.csv")
DEFAULT_STATS_OUTPUT = Path("data/processed/community_stats_enhanced.json")
DEFAULT_FILTER_SUMMARY_OUTPUT = Path("data/processed/community_filter_summary_enhanced.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Gera a versao enhanced do dataset usuario-jogo ja processado."
    )
    parser.add_argument(
        "--base-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Caminho relativo ao base-dir para salvar user_game_edges_enhanced.csv.",
    )
    parser.add_argument(
        "--stats-output",
        type=Path,
        default=DEFAULT_STATS_OUTPUT,
        help="Caminho relativo ao base-dir para salvar community_stats_enhanced.json.",
    )
    parser.add_argument(
        "--filter-summary-output",
        type=Path,
        default=DEFAULT_FILTER_SUMMARY_OUTPUT,
        help="Caminho relativo ao base-dir para salvar community_filter_summary_enhanced.json.",
    )
    parser.add_argument(
        "--min-user-games",
        type=int,
...
def validate_outputs(
    ...
    if not edges_output.exists() or edges_output.stat().st_size == 0:
        raise AssertionError("user_game_edges_enhanced.csv nao foi gerado corretamente")

    csv_head = pd.read_csv(edges_output, nrows=5)
    expected_columns = ["user_id", "app_id"]
    if list(csv_head.columns) != expected_columns:
        raise AssertionError("Colunas inesperadas em user_game_edges_enhanced.csv")

    if not pd.api.types.is_integer_dtype(csv_head["user_id"]):
        raise AssertionError("Tipos inconsistentes em user_game_edges_enhanced.csv")
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mc859-user-game-enhanced-") as temp_dir:
        conn = connect_sqlite(Path(temp_dir) / "enhanced.sqlite")
        try:
            load_summary = load_base_edges(input_path, conn, args.chunk_size)
            filter_summary_base = apply_enhanced_filters(
                conn=conn,
