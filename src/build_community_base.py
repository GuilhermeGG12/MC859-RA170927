...
DEFAULT_EDGES_OUTPUT = Path("data/processed/user_game_edges_base.csv")
DEFAULT_STATS_OUTPUT = Path("data/processed/community_stats_base.json")
DEFAULT_FILTER_SUMMARY_OUTPUT = Path("data/processed/community_filter_summary_base.json")
DEFAULT_CHUNK_SIZE = 1_000_000


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Constroi a base usuario-jogo intermediaria a partir de recommendations.csv."
    )
    parser.add_argument(
        "--base-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Caminho relativo ao base-dir para salvar user_game_edges_base.csv.",
    )
    parser.add_argument(
        "--stats-output",
        type=Path,
...
def build_stats(conn: sqlite3.Connection) -> dict[str, int | float]:
    ...
    if stats["n_edges"] == 0:
        raise AssertionError("user_game_edges_base.csv nao pode ficar vazio")
...
def validate_outputs(
    ...
    if not edges_output.exists() or edges_output.stat().st_size == 0:
        raise AssertionError("user_game_edges_base.csv nao foi gerado corretamente")

    csv_head = pd.read_csv(edges_output, nrows=5)
    expected_columns = ["user_id", "app_id"]
    if list(csv_head.columns) != expected_columns:
        raise AssertionError("Colunas inesperadas em user_game_edges_base.csv")
